(function () {
    ...
    angular
        .module('aditumApp')
        .controller('BudgetExecutionMensualReportController', BudgetExecutionMensualReportController);

    BudgetExecutionMensualReportController.$inject = ['CommonMethods','Company', 'AlertService', '$rootScope', 'Principal', 'MensualAndAnualReport', '$scope', 'Presupuesto', 'globalCompany', 'Modal'];

    function BudgetExecutionMensualReportController(CommonMethods,Company, AlertService, $rootScope, Principal, MensualAndAnualReport, $scope, Presupuesto, globalCompany, Modal) {
        ...
        vm.tableToExcel = function (table) {
            ...
            var workSheetName = vm.companyName +" - EJECUCIÓN PRESUPUESTARIA - del " +moment(vm.fechaInicio).format("L")+" al "+moment(vm.fechaFin).format("L");
        ...
        vm.print = function () {
            ...
            printJS({
                printable: vm.path,
                type: 'pdf',
                modalMessage: "Obteniendo ejecución presupuestaria"
            })
        };
