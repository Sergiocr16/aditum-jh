(function () {
    ...
    angular
        .module('aditumApp')
        .controller('VisitantDialogAdminInvitationController', VisitantDialogAdminInvitationController);

    VisitantDialogAdminInvitationController.$inject = ['Destinies','$localStorage', 'InvitationSchedule', 'VisitantInvitation', '$state', '$timeout', '$interval', '$scope', '$stateParams', 'Visitant', 'House', 'Company', 'Principal', '$rootScope', 'CommonMethods', 'WSVisitorInvitation', 'WSDeleteEntity', 'PadronElectoral', 'globalCompany', 'Modal'];

    function VisitantDialogAdminInvitationController(Destinies, $localStorage, InvitationSchedule, VisitantInvitation, $state, $timeout, $interval, $scope, $stateParams, Visitant, House, Company, Principal, $rootScope, CommonMethods, WSVisitorInvitation, WSDeleteEntity, PadronElectoral, globalCompany, Modal) {
        var vm = this;
...
//            angular.element('#focusMe').focus();
        ...
        function formatVisitor(visitor) {
            ...
            formatPlate();
            visitor.status = 1;
             visitor.houseId = vm.houseSelected;
        ...
        function visitantExistsInBD(data) {
            ...
            Modal.confirmDialog("Un visitante con la cédula " + vm.visitor.identificationnumber + " ya se ha invitado con anterioridad.", " ¿Desea renovar su invitación y actualizar sus datos?", function () {
                ...
                function onSuccess(data) {
                    $localStorage.infoHouseNumber.id = data.houseId;
                    ...
                    if (vm.timeFormat == 1) {
                        ...
                        InvitationSchedule.findSchedulesByInvitation({
                            ...
                            InvitationSchedule.update(invitationSchedule, function () {
                                $state.go('visitant-invited-admin-view');
