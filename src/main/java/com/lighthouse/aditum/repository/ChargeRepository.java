...
public interface ChargeRepository extends JpaRepository<Charge, Long> {
    ...
    List<Charge> findAllBetweenMorosidadDatesAndHouseId(ZonedDateTime finalDate, Long houseId, int deleted, int state);
    ...
    @Query("select c from Charge c " +
        "where  c.date <= ?1 and c.house.id = ?2 and c.deleted=?3 and c.type=?4 and c.state=?5 order by id desc")
    List<Charge> findAllBetweenDatesMorosidadAndHouseIdAndType(ZonedDateTime finalDate, Long houseId, int deleted,int type, int state);
