...
public interface CommonAreaReservationsRepository extends JpaRepository<CommonAreaReservations, Long> {
    ...
    @Query("select e from CommonAreaReservations e " +
        "where e.houseId= ?1 and e.commonArea.id= ?2 and e.status <3 ")
    Page<CommonAreaReservations> findByPendingAndAcceptedToActiveReservationsByHouseIdAndCommonArea(Pageable pageable, Long houseId, Long commonAreaId);
...
//    Page<CommonAreaReservations> findAllByCompanyId(Pageable pageable, Long companyId);
    ...
    @Query("select e from CommonAreaReservations e " +
        "where e.initalDate >= ?1 and e.initalDate <= ?2 and e.company.id = ?3 ")
    List<CommonAreaReservations> findByDatesBetweenAndCompanylist(ZonedDateTime initialDate, ZonedDateTime finalDate, Long companyId);

    @Query("select e from CommonAreaReservations e " +
        "where e.initalDate = ?1  and e.company.id = ?2 ")
    List<CommonAreaReservations> findByDateStartAndCompanylist(ZonedDateTime initialDate, Long companyId);

    @Query("select e from CommonAreaReservations e " +
        "where e.finalDate = ?1  and e.company.id = ?2 ")
    List<CommonAreaReservations> findByDateFinishAndCompanylist(ZonedDateTime initialDate, Long companyId);

    @Query("select e from CommonAreaReservations e " +
        "where e.initalDate >= ?1 and e.initalDate <= ?2 and e.company.id = ?3 and e.status = ?4")
    Page<CommonAreaReservations> findByDatesBetweenAndCompanyAndStatus(Pageable pageable, ZonedDateTime initialDate, ZonedDateTime finalDate, Long companyId, int status);
