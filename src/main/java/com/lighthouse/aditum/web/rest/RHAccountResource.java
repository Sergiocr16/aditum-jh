...
public class RHAccountResource {
    ...
    @GetMapping("/r-h-accounts/findByUserId/{id}")
    @Timed
    public ResponseEntity<RHAccountDTO> getResidentByUserId(@PathVariable Long id) {
        RHAccountDTO rHAccountDTO = rHAccountService.findOneByUserId(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(rHAccountDTO));
    }
